...
export function runSupernova(world, supernovaBody, callbacks) {
  ...
  for (const b of affected) {
    const oldTier = b.tier;
    const newTier = Math.min(SCATTER_MAX_TIER, oldTier + 1);
...
export function runBlackholePhase(world, blackhole, callbacks) {
...
export function runBigBang(world, callbacks) {
  const all = Matter.Composite.allBodies(world);
  for (const b of all) {
    if (b.label !== 'cosmic') continue;
